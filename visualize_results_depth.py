...
#dataset = h5py.File(path+'CSG/out/CSG_dataset_depth0.h5', 'r')
images = dataset['data'];
normals = dataset['label_depth'];
clusters = dataset['clusters_depth'][:];
clusters = np.exp(clusters)
...
for it in range(nb_test):
    ...
    plt.imshow(clusters[normalMax.astype(int)], cmap='jet', norm = norm) #add [:,:,0] for depth
    plt.title('ground_truth')
    frame1 = plt.gca()
    frame1.axes.get_xaxis().set_visible(False)
    frame1.axes.get_yaxis().set_visible(False)
    plt.subplot(1, 4, 3)
    plt.imshow(classifRGB, cmap='jet', norm = norm) #add [:,:,0] for depth
    ...
    frame1.axes.get_xaxis().set_visible(False)
    frame1.axes.get_yaxis().set_visible(False)
    plt.savefig('plots/CSG_depth/test_'+scale+'_BN_iter_'+str(niter)+'_'+str(it)+'.png', dpi=150, bbox_inches='tight')
    plt.show()
