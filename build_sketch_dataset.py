...
files = glob.glob(path_r+'raw_data/*/depth/'+name+'*.png')[5000:]
...
out_normals = np.ndarray((max_size, norm_size[0], norm_size[1]), np.uint8);
out_depth = np.ndarray((max_size, norm_size[0], norm_size[1]), np.uint8);
out_images = np.ndarray((nb_trial, max_size, 4, input_size[0], input_size[1]), np.uint8);
    ...
    for i in range(min(max_size, nimg-n*max_size)):
        ...
        if not path.exists(dir_name_res+'normal/'+img_name) \
           or not path.exists(dir_name_res+'depth/'+img_name) :
            ...
            for t in range(nb_trial):
                out_images[t,i] = io.imread(dir_name_res+'T'+str(t+1)+'/'+img_name).transpose(2,0,1);
